...
export const API_URL = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4040';
...
export const gmailApi = {
  /**
   * Get the Google consent URL to open. `platform: 'web'` makes the backend
   * callback redirect back to `/settings?gmail=connected` after consent.
   */
  connectUrl: () => apiFetch<{ url: string }>('/integrations/gmail/connect?platform=web'),
 ...
 * picks the rail; both are sent to `phone` from the shared account. `templateId`
