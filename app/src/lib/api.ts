...
export const authApi = {
  ...
  /**
   * Exchange the one-time `handoff` token (delivered on the deep link by the
   * Google callback redirect) for the real JWT pair. No auth header - the
   * handoff IS the credential.
   */
...
export const googleImportApi = {
   ...
   * 'app'` returns via the `circlethedate://google-import-connected` deep link.
   */
  connectUrl: () =>
    apiFetch<{ url: string }>('/integrations/google-import/connect?platform=app'),
 ...
 * of their birthday. Only the Google import produces these (Stage 16); CSV/contacts
